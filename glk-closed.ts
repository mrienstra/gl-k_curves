 ...
 */
export function cosSeamT(copies: number): number {
 ...
 */
export function findSeamT(coeffs: number[][], p0: number[], copies: number): number {
...
function sampleSymmetric(coeffs: number[][], seamT: number, nSamples: number): number[][] {
  const out: number[][] = [];
 ...
 */
export function computeAutoSeamT(pts: number[][], copies: number, k = 1): number {
 ...
 *
 * @param pts      - control points; first and last must be the same point
 *                   (the coincident seam point produced by "Close segment")
 * @param k        - GL-k order (0, 1, 2, …)
 * @param nSamples - number of output samples (for the middle copy)
 * @param opts     - optional per-segment config (overrides window.closedCurve)
 * @returns approximately-closed polyline (first ≈ last point)
 */
export function sampleGLKClosed(pts: number[][], k = 1, nSamples = 200, opts: ClosedCfg | null = null): number[][] {
  if (pts.length < 3) return [];

  const cfg: ClosedCfg = opts ?? (typeof window !== 'undefined' ? window.closedCurve : undefined) ?? {};
 ...
 */
function _sampleClosedWithMatrix(
  pts: number[][],
  matrixFn: (n: number) => Float64Array[],
  nSamples: number,
  opts: ClosedCfg | null = null,
): number[][] {
  const cfg: ClosedCfg = opts ?? (typeof window !== 'undefined' ? window.closedCurve : undefined) ?? {};
  let copies = Math.max(2, Math.round(cfg.copies ?? 3));
  if (copies % 2 === 0) copies++; // must be odd so the middle copy has equal context on both sides
...
export function sampleGLKFractionalClosed(pts: number[][], k = 1, nSamples = 200, opts: ClosedCfg | null = null): number[][] {
  if (pts.length < 3) return [];
...
export function sampleModifiedGLKClosed(
  ...
  return _sampleClosedWithMatrix(
    pts,
    (n) => {
      const w0 = (eta1 === null || eta2 === null) ? glWeights(n)[0] : 0;
      const e1 = eta1 !== null ? eta1 : 1 / w0;
      const e2 = eta2 !== null ? eta2 : 1 / w0;
