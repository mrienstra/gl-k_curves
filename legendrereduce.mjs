...
export function legendreReduce(k, f) {
  ...
  let sigE = isVec ? new Array(f[0].length).fill(0) : 0;
  let sigO = isVec ? new Array(f[0].length).fill(0) : 0;
...
function add(a, b, isVec) {
  if (!isVec) return a + b;
  return a.map((v, i) => v + b[i]);
}

function scale(s, v, isVec) {
  if (!isVec) return s * v;
  return v.map(x => s * x);
}
