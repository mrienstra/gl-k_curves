...
function R(e) {
	let t = [], n = /* @__PURE__ */ new Set(), r = [], i = /([MLmZz])((?:[^MLmZz])*)/g, a;
	for (; (a = i.exec(e)) !== null;) {
		let e = a[1], i = L(a[2]);
		if (e === "M") {
			r.length > 0 && (t.push(r), r = []);
			for (let e = 0; e + 1 < i.length; e += 2) r.push([i[e], i[e + 1]]);
		} else if (e === "L") for (let e = 0; e + 1 < i.length; e += 2) r.push([i[e], i[e + 1]]);
		else if (e === "m") {
			for (let e = 0; e + 1 < i.length; e += 2) if (i[e] === 0 && i[e + 1] === 0 && r.length > 0) {
				let e = r[r.length - 1];
				t.push(r), r = [[...e]];
			}
		} else (e === "Z" || e === "z") && r.length > 0 && (r.push([...r[0]]), n.add(t.length), t.push(r), r = []);
	}
	return r.length > 0 && t.push(r), {
		segments: t,
		closedSet: n
	};
}
function z(e) {
	...
	return {
		d: t.d ?? "",
		k: t.k == null ? void 0 : Number(t.k),
		modified: t.modified == null ? void 0 : t.modified !== "false",
		eta: t.eta == null ? void 0 : t.eta === "auto" ? null : Number(t.eta),
		alpha: t.alpha == null ? void 0 : Number(t.alpha),
		stroke: t.stroke,
		strokeWidth: t["stroke-width"] == null ? void 0 : Number(t["stroke-width"])
	};
}
...
function V(e, t = {}) {
	let n = typeof e == "string" ? z(e) : e, r = n.k ?? 1, i = n.modified ?? !0, a = n.eta === void 0 ? null : n.eta, o = n.alpha ?? 1, s = n.stroke ?? "#333333", c = n.strokeWidth ?? 2, { padding: l = 10, background: u } = t, { segments: d, closedSet: f } = R(n.d), p = Infinity, m = Infinity, h = -Infinity, g = -Infinity;
	for (let e of d) for (let [t, n] of e) t < p && (p = t), t > h && (h = t), n < m && (m = n), n > g && (g = n);
	if (!isFinite(p)) return "<svg xmlns=\"http://www.w3.org/2000/svg\" width=\"0\" height=\"0\"></svg>";
	let _ = p - l, v = m - l, y = h - p + 2 * l, b = g - m + 2 * l, x = I(d, B(r, i, a, o, s, c, h + l, g + l, f));
	return x = x.replace(/width="[^"]*" height="[^"]*" viewBox="[^"]*"/, `width="${y}" height="${b}" viewBox="${_} ${v} ${y} ${b}"`), x = u ? x.replace(/style="background:[^"]*"/, `style="background:${u}"`) : x.replace(/ style="background:[^"]*"/, ""), x;
}
//#endregion
export { R as parsePath, V as renderGLKPath };
